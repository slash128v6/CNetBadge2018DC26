...
unsigned long buttonEvents[NUMBTNS];
byte buttonStatuses[NUMBTNS];
byte buttonmap[NUMBTNS] = {BRIGHTBTN, MODEBTN};
...
void doButtons() {
  ...
  switch (buttonStatus(0)) {
    ...
    case BTNLONGPRESS: // button was held down for a while
      ...
      confirmBlink(); // two green blinks: auto mode. two red blinks: manual mode.
      eepromMillis = currentMillis;
      eepromOutdated = true;
      break;
